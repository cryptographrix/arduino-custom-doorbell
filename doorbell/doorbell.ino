...
// buttonPin doorbellPin lastButtonState lastDebounceTime dingDong lightPin
int bells[4][6] = {
    { 3, 6, LOW, 0, 0, 10 },  // 0 - First Floor
    { 4, 7, LOW, 0, 0, 11 },  // 1 - Second Floor
    { 5, 8, LOW, 0, 0, 12 },  // 2 - Basement
    { 9, 6, LOW, 0, 0, 13 }  // 3 - Placeholder for "ring all"
};
...
int DingDong (int bellsArrayMember) {
      // This is really bad but it works for what it needs to do
      // First I set up the pins....
      int pin = bells[bellsArrayMember][1];        // This pin is what to cause noise through
      int BlinkPin = bells[bellsArrayMember][1];   // This pin modifies lighting status of the doorbell

      // I am not using ding() and dong() for
      // just what they were intended for.
      // This is disgusting and will be changed
      // in a future revision.
      dong(BlinkPin);       // Here, we turn off the light on the doorbell button
      ding(pin);            // Then we ding the doorbell
      delay(heldDownMillis);
      ding(BlinkPin);       // Then we turn the light on on the doorbell button
      dong(pin);            // Then we dong the doorbell
